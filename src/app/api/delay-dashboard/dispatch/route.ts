...
import {
  ...
  type RawRow,
  type StageKey,
} from "@/lib/server/dispatch-report";

export const runtime = "nodejs";
export const dynamic = "force-dynamic";

// The Dispatch Report view lives inside the Delay Dashboard module (its
// own tab, beside Report / NDD Shift Report / PID Vault) — this route is
// a sibling under /api/delay-dashboard/, same pattern as pid-masters's
// own sibling route.
...
const PAGE_SIZE = 500;
// Safety ceiling, not a realistic expectation — see cl/route.ts's
// identical fetchDeptStage for why: a fixed low page cap silently
...
function responseError(error: unknown) {
  ...
  console.error("[delay-dashboard/dispatch] Request failed.", error);
  if (upstreamStatus === 401 || upstreamStatus === 403) {
    return jsonResponse({ ok: false, error: "NexS authentication is unavailable or has expired." }, 401);
  }
  return jsonResponse({ ok: false, error: "Dispatch report data is temporarily unavailable. Please retry." }, 502);
}
...
export async function POST(request: Request) {
  try {
    ...
    const key = cacheKey("delay-dashboard-dispatch", { facility, nddOnly, jit, international, dateRange });
